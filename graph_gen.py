        ...
        for j in range(i + 1, len(boxes)):
            ...
            if not (bound_a.top_right.x < bound_b.bottom_left.x or bound_a.bottom_left.x > bound_b.top_right.x or
                ...
                if overlap_percentage >= overlap_threshold:
                    ...
                    text = ax.text(text_x, text_y, team_a, fontsize='small', fontstretch='ultra-condensed')
                        ...
                        for coord in coords:
                            ...
                            if not readjust:
                                ...
                                arrow = ax.annotate('', xy=(arrow_x, arrow_y), xytext=coord,
                                            arrowprops=dict(arrowstyle='-', lw=1), fontsize='small',
                                            fontstretch='ultra-condensed')
                                text = ax.text(coord[0], coord[1], team_a, fontsize='small', fontstretch='ultra-condensed')
                                break
...
def generate(x, y, text=False):
    ...
    plt.title(f'{x_verbose} ({x_abbrev}) vs {y_verbose} ({y_abbrev}) -- {d1}')
    # plt.tight_layout()
